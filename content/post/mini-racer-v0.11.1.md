...
delayed callbacks from `setTimeout`. In other words, if we want to be able to
use `setTimeout` to schedule work for `N` seconds from now, and have it
actually, you know, _run_ that delayed work, we need to convince V8 to actually
run, continually, until explicitly shut down.
